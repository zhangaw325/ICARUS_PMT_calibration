...
void FitChargeDistributions_LC_batch(string pmtRow,
  ...
  for(int i=0; i<4; i++){
    resultnames[i]  = strchimney + strpmt + "CH" + channelnames[i] + "_"
      + "Led" + ledstr + "_lowcharge.pdf";
    cout << resultnames[i] <<endl;
  }
    
  string outnameroot = strchimney + strpmt + "lowcharge_gain_.root";
  string outnametxt = strchimney + strpmt + "lowcharge_gain_fit.txt";
  ...
  for(int i = 0; i < 4; i++){
	...
	+"chID\t"+to_string(i)+"\t"  //channel id
	+fitbeginch[i][k]+"\t"          //start fit
	+fitendch[i][k]+"\t"            //end fit
	+rebinfactor[i][k]+"\t"         //rebin factor
	+mu_0[k]+"\t"                  //mu
	+q_0[k]+"\t"                   //q
	+sigma_0[k]+"\t"               //sigma
	+amp_0[k];  //amplitude
