...
from ROOT import TCanvas, TGraph, TH1F, TFile, TH2F, TLine
import peakutils
...
Nwaves = len(open(filelistStr).readlines()) / \
    4  # unclear why this is divided by 4
    ...
    for ch in range(NCH):
        for waveNb in range(Nwaves):
            ...
            peakindex = peakutils.peak.indexes(
                -1.0 * awave[0:NSamples], thres=abs(threshold), min_dist=31, thres_abs=True)
            ...
            # print ch, waveNb, peakindex for every hundredth waveform
            if waveNb % 100 == 0:
                print ch, waveNb, afilename
                for bin in range(0, NSamples, 1):
                    #hWave_list[ch].SetBinContent(
                        #bin + 1, -1000.0 * (awave[bin] - baseline_mean))	# subtract off baseline from each data point
                    hWave_list[ch].SetBinContent(bin+1,awave[bin])			# write data points directly
                waveDir.cd()
                ...
                hWave_list[ch].Write()						# store the waveform

                print baseline_mean,baseline_width,threshold
