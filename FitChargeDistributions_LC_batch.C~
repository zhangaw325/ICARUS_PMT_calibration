...
void FitChargeDistributions_LC_batch(string pmtRow,
  ...
  */
  ...
  double parerr[NPAR_i];
    ...
    fit_ideal_1->SetParameters(mu_0,q_0,sigma_0,hCharge[0]->Integral()/2);
    fit_ideal_2->SetParameters(mu_0,q_0,sigma_0,hCharge[0]->Integral()/2);
    fit_ideal_3->SetParameters(mu_0,q_0,sigma_0,hCharge[0]->Integral()/2); 
    ...
    for(int j = 0; j < 3; j++){
      ...
      gStyle->SetOptFit();
      if(j==0)
	hCharge[0]->Fit("fit_ideal_1","","",fitbeginch[i],fitendch[i]);
      else if(j==1)
	hCharge[1]->Fit("fit_ideal_2","","",fitbeginch[i],fitendch[i]);
      else
	hCharge[2]->Fit("fit_ideal_3","","",fitbeginch[i],fitendch[i]);
    }
